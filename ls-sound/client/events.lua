...
RegisterNUICallback('updateDuration', function(data, cb)
    if data and data.id and data.duration then
        if LSSound.SoundInfo[data.id] then
            LSSound.SoundInfo[data.id].duration = data.duration
        end
    end
    cb('ok')
end)

RegisterNUICallback('updateCurrentTime', function(data, cb)
    if data and data.id and data.time then
        if LSSound.SoundInfo[data.id] then
            LSSound.SoundInfo[data.id].currentTime = data.time
        end
    end
    cb('ok')
end)
...
RegisterNUICallback('updateInfo', function(data, cb)
    if data and data.id and data.info then
        if LSSound.SoundInfo[data.id] then
            LSSound.SoundInfo[data.id].info = data.info
        end
    end
    cb('ok')
end)

RegisterNetEvent('ls-sound:seek')
AddEventHandler('ls-sound:seek', function(id, position)
    LSSound.Seek(id, position)
end) 
